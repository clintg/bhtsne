...
def tsne(data, dimensions=2, perplexity=30.0, theta=0.5, max_iter=1000, rand_seed=-1, seed_positions=np.array([])):
    tsne = BHTSNE()
    ...
    Y = tsne.run(data, data.shape[0], data.shape[1], dimensions, perplexity, theta, max_iter, rand_seed, seed_positions, skip_random_init)
    return Y
